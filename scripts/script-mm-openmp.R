...
# rodar assim:
# Rscript script-mm-openmp.R teste-openmp-mm
...
dados %>%
    ...
    group_by(App, Caso, Threads, Matriz
     ...
     summarize(Media = mean(Tempo), SD = sd(Tempo), N = length(unique(Repeticao))) %>% write.csv("media-mm-openmp.csv")
...
ggsave("grafico-mm-openmp.pdf", plot = grafico, width = 8, height = 4, units = "in")
