...
dados %>%
    ...
    group_by(App, Caso, Threads, Matriz
     ...
     summarize(Media = mean(Tempo), SD = sd(Tempo), N = length(unique(Repeticao))) %>% write.csv("media-merge-starpu.csv")
...
ggsave("grafico-merge-starpu.pdf", plot = grafico, width = 8, height = 4, units = "in")
