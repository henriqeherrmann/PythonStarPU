...
# rodar assim:
# Rscript script-gauss-starpupython.R teste-starpu-gauss
...
list.files(path = diretorio, pattern = "result*", full.names=TRUE, recursive = TRUE),   # lista todos que comecam com result
    function(x){
        ...
        linhaTempo <- grep("Tempo paralelo", readLines(x), value = TRUE)
        tempo <- gsub(" ", "",(strsplit(linhaTempo, split = ":", fixed = TRUE) %>% unlist)[2])
        tibble(
            App = app,
            Caso = caso, 
...
dados %>%
    ...
    group_by(App, Caso, Threads, Matriz
     ...
     summarize(Media = mean(Tempo), SD = sd(Tempo), N = length(unique(Repeticao))) %>% write.csv("media-gauss-starpupython.csv")
...
ggsave("grafico-gauss-starpupython.pdf", plot = grafico, width = 8, height = 4, units = "in")
